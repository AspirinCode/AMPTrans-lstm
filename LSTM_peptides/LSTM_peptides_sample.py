...
flags.add_argument("-t", "--train", default=False, help="whether the network should be trained or just sampled from", type=bool)
...
flags.add_argument("-a", "--startchar", default="j", help="starting character to begin sampling. Default='j' for 'begin'", type=str)
...
flags.add_argument("--finetune", default=True, help="if True, a pretrained model provided in modfile is finetuned on the dataset", type=bool)
...
def _onehotencode(s, vocab=None):
    ...
    : j   init char     " " pading char
    """
    if not vocab:
        vocab = ['j','A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', ' ']
...
class SequenceHandler(object):
    ...
    def pad_sequences(self, pad_char=' ', padlen=0):
        ...
        if padlen:
            ...
            padded_seqs = []
            for seq in self.sequences:
                padded_seq = 'j' + seq + pad_char * (length - len(seq))
                padded_seqs.append(padded_seq)
    ...
    def analyze_generated(self, num, fname='analysis.txt', plot=False,min_length_seq=3):
        ...
        with open(fname, 'w') as f:
            ...
            f.write("Number of invalid (with j):\t%i\n" % (len1 - len2))
            ...
            self.sequences = [s[1:].rstrip() for s in self.sequences]

            self.analyze_training()

            d.sequences = [s for s in d.sequences]
            
            descriptor = 'pepcats'
            #seq_desc = PeptideDescriptor([s[1:].rstrip() for s in self.sequences], descriptor)
            seq_desc = PeptideDescriptor(self.sequences, descriptor)
            seq_desc.calculate_autocorr(min_length_seq)

            gen_desc = PeptideDescriptor(d.sequences, descriptor)
            gen_desc.calculate_autocorr(min_length_seq)
            ...
            ran_desc.calculate_autocorr(min_length_seq)
            ...
            hel_desc.calculate_autocorr(min_length_seq)
...
class Model(object):
    ...
    def initialize_model(self, seed=42):
        ...
        optimizer = Adam(lr=self.lr, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0) #learning_rate
    ...
    def sample(self, num=100, minlen=7, maxlen=50, start=None, temp=2.5, show=True):
...
def main(infile, sessname, neurons=64, layers=2, epochs=100, batchsize=128, window=0, step=1, target='all',
         valsplit=0.2, sample=100, aa='j', temperature=2.5, cell="LSTM", dropout=0.1, train=False, learningrate=0.01,
         modfile=None, samplelength=36, pad=0, l2_rate=None, cv=None, finetune=True, references=True):
    ...
    data.generated = model.sample(sample, start=aa, maxlen=samplelength, show=True, temp=temperature)
...
if __name__ == "__main__":
    ...
    main(infile=args.dataset, sessname=args.name, batchsize=args.batch_size, epochs=args.epochs,
         ...
         temperature=args.temp, dropout=args.dropout, train=False, modfile=args.modfile,
         ...
         finetune=False, references=args.refs)
